...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    * Отправляет сообщение об ошибке в группу администрирования
    """
    try:
        ...
        logger.error("Was not able to return user error message")
    ...
    for message_part_idx, message_part in enumerate(messages_parts):
        ...
        if len(messages) > 0 and (len(messages[-1]) + curr_len <= 4096):
            ...
            messages += [
                template.format(
                    message_part=message_part[message_part_text_start_idx : message_part_text_start_idx + 4096]
                )
                for message_part_text_start_idx in range(0, curr_len, 4096)
            ]

    for message in messages:
        await context.application.bot.send_message(settings.tg_admin_chat, message, parse_mode=ParseMode.HTML)
